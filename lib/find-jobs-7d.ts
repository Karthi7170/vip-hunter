...
function clearlyRequiresTwoPlusYears(text: string) {
  ...
  return [...source.matchAll(/\b(\d+)\s*\+\s*(?:years?|yrs?)\b/g)]
...
async function scanLever(board: { slug: string; name: string }): Promise<Job[]> {
  ...
  for (const posting of postings) {
    if (!posting.createdAt || !withinLast7Days(posting.createdAt)) continue;
    ...
    jobs.push({
      ...
      type,
      fit,
      posted: new Date(posting.createdAt).toISOString(),
...
async function scanSmartRecruiters(board: { id: string; name: string }): Promise<Job[]> {
  ...
  const candidates = (list.content || []).filter((posting) => {
    if (!posting.releasedDate || !withinLast7Days(posting.releasedDate)) return false;
  ...
  details.forEach((result, index) => {
    ...
    const description = stripHtml(
      Object.values(posting.jobAd?.sections || {}).map((section) => section?.text || "").join(" "),
    );
    ...
    jobs.push({
      ...
      type,
      fit,
      posted: new Date(releasedDate).toISOString(),
      matchedSkills: matched,
      missingSkills: missing,
      requirements: [...new Set([...matched, ...missing])],
      whyFit: buildWhyFit(type, matched, missing),
